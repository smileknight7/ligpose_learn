...
# 这里也可以改一下，先构建dataset，再进行划分
    ...
    # 验证数据是否能重新加载
    # if DEBUG_ONLY_VAL:
    #     print("DEBUG_ONLY_VAL is set to True, only running validation once.")
    ...
    #             if dic_data is None:
    #                 print(f"[DEBUG_ONLY_VAL] Skipping None data at index {i}跳过")
    #                 continue
    #             dic_data = dic_data.to(rank)
    #             tup_pred = my_model(dic_data)                           # 这里开始训练
    #             grad_loss, eval_loss = loss_object(tup_pred, dic_data, epoch=0)
    #             print(f"[DEBUG_ONLY_VAL] eval_loss: {eval_loss}")
    ...
    # wandb init
    if rank == 0 or not args.use_multi_gpu:
        wandb.init(project="LigPose-Struct-semi", name=f"semi_rate_{args.semi_rate}_mask_l_{args.mask_rate_l}_mask_p_{args.mask_rate_p}_frag_{args.fragment}_seed_{args.seed}",
                   config=vars(args))
        wandb.watch(my_model, log="all", log_freq=100)


    # traj visualization   训练日志初始化
    if rank == 0 or not args.use_multi_gpu:
    ...
    # Training
    ####################################################################################################################
    print(f"[DEBUG] rank is: {rank if 'rank' in locals() else 'NOT_DEFINED'}")          # 验证rank是否被初始化
    if rank == 0 or not args.use_multi_gpu:
    ...
    for epoch in range(args.restart, args.n_epoch):
        ...
        if rank == 0 or not args.use_multi_gpu:
            ...
            progBar = Progbar(len(train_loader))                                 # len(train_loader)------>每一轮epoch中batch的数量
            dic_train = defaultdict(list)                                               # 按键聚合数据
        for i, dic_data in enumerate(train_loader):
            ...
            if args.use_amp_mix:
                ...
                with torch.cuda.amp.autocast(enabled=True, dtype=torch.float16):
                    tup_pred = my_model(dic_data)                                       # 将数据传入模型    使用的是这个dic_data！！！！打包为PYG的数据
        ...
        # =================== Val ===================
        my_model.train(False)                                       # 按照上面写的应该是1个epoch结束之后会进行一次val
        loss_object.train(False)
        if args.use_multi_gpu:
        ...
        if rank == 0 or not args.use_multi_gpu:
            ...
            current_lr = opt_object.optimizers[0].param_groups[0]['lr']
        ...
        # ================== wandb log ===================
            train_log = {f"train/{k}": float(np.mean(dic_train[k])) for k in dic_train.keys()}
            val_log   = {f"val/{k}":   float(np.mean(dic_val[k]))   for k in dic_val.keys()}
            log_dict = {
                **train_log,
                **val_log,
                "lr": float(current_lr),
            ...
            print("WILL_LOG_TO_WANDB:", log_dict)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vocab_processed_path', type=str,
                        default='/data/lpw/ligpose/data/work_file/pdbbind_listvocab.pkl',   # 
                        help='index_to_pdbbind_refined')
    parser.add_argument('--f_pkl_path', type=str,
                        default='/data/lpw/ligpose/data/work_file/fragment_data/fragment_data',
                        help='fragment pos path')
    parser.add_argument('--fragment', type=bool,
                        default=False,
                        help='if use fragment-based position')
    # data source (pdbbind)
    parser.add_argument('--pdbbind_path', type=str,                                         # 预处理部分保存的文件
...
#以下是修改内容
    ...
    parser.add_argument('--core_list_path', type=str,
                        default='None', help='core set of PDBbind')
    #这个core_list可能是比较重要的还是要多进行考虑
...
#blind training是指测试集中的样本在训练过程中是完全不可见的，而不是从原本的数据集划分而来
    ...
    #这个origin_path好像不是很重要，在脚本中并没有用过这个变量（应该就是指向一个原始数据？）
    ...
    parser.add_argument('--update_together', type=str, default=False, help='if update grad together')           # 这个是什么东西呢
...
#########################这里添加对rank这个参数赋值的内容################
    ...
    parser.add_argument('--weight_path', type=str, default='./weights/semi', help='path to save model')
