...
#这里数据集这里还是有点问题，没有排除之前的core_set应该是
#################################################
########-----------划分数据集------------##########
#################################################
...
#这里的semi_rate对数据
    def __getitem__(self, i):
        if self.mode == 'train':
            ...
            #self.semi_rate是设置一个比例，根据概率来选择从哪个列表中选取文件，这种采用的应该是mix sampling 的方式，进行混合采样
            #这里采样的过程就是使用半监督的啊
...
#此处是进行采样，分为了有标签数据和无标签数据
    ...
    def get_complex(self, idx):
        if isinstance(idx, tuple):  #判断idx是不是元组类型
            # for select pair, (l_idx, r_idx)
            idx, rand_idx = idx  # ligand, protein
            #这里是进行了解包操作，将idx分为了ligand和rand_idx两部分分别代表ligand和protein部分
            #但是这里并没有指定那一部分应该分给谁，所以前面idx生成的时候应该是已经定义过的，第一个元组给ligand，而第二个给protein
        else:
            rand_idx = None
