...
from utils.pdbbind_preprocess import process_semi_pocket, process_semi_ligand,try_prepare_pdbbind

#定义task作为process的传入变量
#将task任务进行拆包并提取内部的元素
#原本的脚本，这个脚本的输出路径是错误的会有覆盖，而且没有构成蛋白质和配体之间的配对关系
# def process(task):
...
#构建tasks列表，将使用的函数，传入的参数交给多进程池
    ...
    for c in os.listdir(args.data_path):
        tasks.append((process, (c, args.data_path, args.protein_output_path,args.ligand_output_path,args.complex_output_path,args.cache)))
