...
def tree_decomp(mol):
    ...
    for i in range(len(cliques)):               # 团簇索引
        for atom in cliques[i]:                 # 团簇i中的原子索引
            nei_list[atom].append(i)            # 根据这个原子索引添加团簇索引
    ...
    for i in range(len(cliques)):
        ...
        for atom in cliques[i]:
            for j in nei_list[atom]:
                if i >= j or len(cliques[j]) <= 2: continue  # if i >= j是为了避免重复检查
                inter = set(cliques[i]) & set(cliques[j])
                if len(inter) > 2:                           # 其实这种情况比较少，化学上很少出现共用两个原子以上的环
                    cliques[i].extend(cliques[j])            # 共用原子超过两个的时候将两个环合并，并去掉其中一个环
    ...
    for atom in range(n_atoms):
        if len(nei_list[atom]) <= 1: 
            continue
        cnei = nei_list[atom]                                   # 当前原子的团簇索引列表
        bonds = [c for c in cnei if len(cliques[c]) == 2]       # 当前原子的bond索引列表（与之成键原子的数量）
        rings = [c for c in cnei if len(cliques[c]) > 4]        # 当前原子的ring索引列表（与之成大于4的环的数量）
        if len(bonds) > 2 or (len(bonds) == 2 and len(cnei) > 2): #In general, if len(cnei) >= 3, a singleton should be added, but 1 bond + 2 ring is currently not dealt with.
            ...
            cliques.append([atom])                              # 在团簇上增加一个孤立节点
            c2 = len(cliques) - 1
            for c1 in cnei:
    ...
    edges = [u + (MST_MAX_WEIGHT-v,) for u,v in edges.items()]  # 列表推导式edges.items()先返回一个元组对象，随后解包为列表
    if len(edges) == 0:
        return cliques, edges, nei_list

    #Compute Maximum Spanning Tree
    row,col,data = zip(*edges)                              # edges是一个包含三元组的列表，(c1, c2, weight)
    n_clique = len(cliques)
    clique_graph = csr_matrix( (data,(row,col)), shape=(n_clique,n_clique) )
    junc_tree = minimum_spanning_tree(clique_graph)
    row,col = junc_tree.nonzero()
    edges = [(row[i],col[i]) for i in range(len(row))]      # 顺序不变，只是打包在一起了
