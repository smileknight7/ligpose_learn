...
from pdbbind_utils import collate_dummy, pred_ens, assign_struct
from common import *
...
#   ligpose = LigPose(param_path=param_path)这里由于一直要提示加载struct_args，不能识别我本身传入的args参数
#很怪，这个要再好好debug一下，看下这里是怎么做到的呢
    params = torch.load(param_path, map_location='cpu')
    args = params.get('struct_args', params['args'])
    ligpose = LigPose(args=args)
    ligpose.ligpose_struct.load_state_dict(params['model_state_dict'], strict=True)
    return ligpose
...
def prepare_single_input(tupin, dis=15):
    ...
    df_pocket, _ = get_pocket(df_protein, ref_pocket_center, dis=dis, any_atom=True)
    biodf_protein.df['ATOM'] = df_pocket
...
class InferDataset(torch.utils.data.Dataset):
    ...
    def get_complex(self, idx):
        ...
        # load prepared data
        ################################################################################################################
        dic_data = np.load(f'{self.cache_path}/{idx}')
        ...
        node_cycling_loc = np.concatenate([np.arange(len(protien_core_atom_loc)),       # 这个可以用于构建mask，但是没有使用
...
def predict(
        ...
        seed=42,
        min=True,
        args=True,#这里传入args会不会引导后面进行args还是struct_args的判断呢？
    ):
    set_device(device)
    set_all_seed(seed)
...
#     parser.add_argument('--param_path', default='/home/lpw/ligpose_learn/weights/semi/state_15.chk', type=str, required=True, help='')
#     parser.add_argument('--protein', default='/home/lpw/ligpose_learn/example/example_files/3g0g/3g0g_ligand.mol2', type=str, nargs='*', help='protein_path')
#     parser.add_argument('--ligand', default='/home/lpw/ligpose_learn/example/example_files/3g0g/3g0g_ligand.mol2', type=str, nargs='*', help='ligand_path')
#     parser.add_argument('--ref_pocket_center', type=str, nargs='*', help='reference pocket center')
#     parser.add_argument('--batch_csv', type=str, default=None, help='batch_csv')
#     parser.add_argument('--cache_path', type=str, default='./cache', help='cache_dir')
#     parser.add_argument('--output_structure_path', default='./output', type=str, default='./output_structure', help='output_structure_path')
#     parser.add_argument('--output_result_path', default='./output', type=str, default=None, help='output_csv')
#     parser.add_argument('--device', type=str, default='cpu', help='inference device')
#     parser.add_argument('--ens', default=5, type=int, default=1, help='?')
#     parser.add_argument('--batch_size', type=int, default=1, help='batch size')
#     parser.add_argument('--num_workers', type=int, default=8, help='data loader workers')
#     parser.add_argument('--pred_type', default='struct', type=str, default=None, help='prediction type')
#     parser.add_argument('--seed', type=int, default=42, help='random seed')
#     parser.add_argument('--min', action='store_true', help='if minimize ligand structure before saving')
#     parser.add_argument('--prepare_data_with_multi_cpu', action='store_true', help='mulit-cpu for data preparation')
...
#         ligand=args.ligand,
#         ref_pocket_center=args.ref_pocket_center,
#         batch_csv=args.batch_csv,
#         prepare_data_with_multi_cpu=args.prepare_data_with_multi_cpu,
