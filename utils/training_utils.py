...
def train_with_args(args, train):  #训练入口
...
class CustomOptimization():
    ...
    def __call__(self, grad_loss, update_together=True, epoch=-1):        # call函数，直接在实例上传参数就可以进行使用了
        '''
        opt model params
...
#这里缺少了构建参数struct_args,但是按道理我应该找到传入这个args的接口应该就可以进行predict了
...
#设置初始化的模型参数，包含了继续训练中的断点重连内容
def init_params(args, my_model, loss_object, opt_object):           #初始化模型参数
    if args.restart == 0:
