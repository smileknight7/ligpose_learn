...
##########-----------------------------###################################
######这边代码还要再进行修改一下，要尝试进行，保存数据集划分，加载数据集以及检验数据集
#########------------------------------###################################



###########################这里应该再进行添加,需要检查一下是不是这样进行修改###########

#这里是定义了用在train中检查数据集是否进行划分，
def check_data_split(path='./'):
    return os.path.exists(f'{path}/train_list_labeled') \
           and os.path.exists(f'{path}/val_list_labeled') \
           and os.path.exists(f'{path}/test_list_labeled') \
           and os.path.exists(f'{path}/train_list_unlabeled')
           

#这里是原本的项目中划分数据集并加载数据集的函数
