...
class GateResidue(torch.nn.Module):                                             #----->__init__内定义的变量是在周期内需要保存的层/权重，如果是一直在改变的变量是不需要定义的
    ...
    def forward(self, x, res):
        if self.gate_flag:
            g = self.gate(torch.cat((x, res, x - res), dim=-1)).sigmoid()           #这里使用g来作为门控实现残差连接（g属于0~1之间），如果res很小则主要保留x，如果res很大那倾向于保留res
...
class SE3EquivariantAttention(torch.nn.Module):
    def __init__(self,
        ...
        self.n_head = n_head
        self.head_hidden = head_hidden                                      # node_hidden // args.n_head
        self.sqrt_head_hidden = np.sqrt(self.head_hidden)                   #注意力机制中的缩放因子
...
#SE（3）是一个几何对称群，表示三维空间中的旋转和平移，等变性意味着模型的输入会随着，
        ...
        self.lin_qk = torch.nn.Linear(node_hidden, n_head * head_hidden * 4)           #这里是先将特征映射到4倍，随后又进行chunk拆分为4个张量        
...
#辅助性组件
    ...
    def forward(self, complex_graph):
        x = complex_graph.x                                             #这个就是节点node属性
        coor = complex_graph.coor
        edge_attr = complex_graph.edge_attr

        rel_coor = rearrange(coor, 'b i d -> b i () d') - rearrange(coor, 'b i d -> b () i d') #利用广播机制计算距离dismap             # [B, N, N, D]  这里用法相当于是.unsqueeze(1)
        rel_dist = self.dist_scale(rel_coor.norm(p=2, dim=-1))       # dismap [B, N, N]------> [B, N, N, RBF_num_gaussians]  这里是计算欧式距离并映射到了高维空间中

#边特征处理，将特征组织为多头形式，方便计算注意力

        l_m, r_m, l_v, r_v = self.lin_qk(x).chunk(4, dim=-1)        #_m这种是注意力打分分支，_v是信息传递分支
                                                                    # [B, N, 4*H*D] ----> [B, N, H*D] ---->4个这种形状的张量
        l_m, r_m, l_v, r_v = map(lambda t: rearrange(t, 'b n (h d) -> b n h d', h=self.n_head), (l_m, r_m, l_v, r_v))
                                                                                                            # [B, N, H, D]   这里是将4块每一块都拆分8个注意力头                                                           
#边特征与距离信息融合 l_m就是表示左侧的key
        edge_coor = torch.cat((edge_attr, rel_dist), dim=-1)
        #print(edge_attr.shape, rel_dist.shape)
        e_m, e_v = self.edge_coor_to_att(edge_coor).chunk(2, dim=-1)
        e_m, e_v = map(lambda t: rearrange(t, 'b i j (h d) -> b i j h d', h=self.n_head), (e_m, e_v))      #拆分注意力头
...
# einops 库中的重排模式表示法b - batch 批次维度
        ...
        # do attention----->将节点特征构建成边特征的形状
        l_m = rearrange(l_m, 'b i h d -> b i () h d')                                       # [B, N, 1, H, D]  通过广播机制进行赋值
        r_m = rearrange(r_m, 'b j h d -> b () j h d')
        att = l_m * r_m * e_m                                                               #这里是在计算attentionl_m * r_m 相当于是K和Q
        att_sum = att.sum(dim=-1) / self.sqrt_head_hidden                                   # [B, N, N, H]  这里是将每个头的特征进行求和得到注意力分数
        att_mask = (rearrange(complex_graph.edge_mask_after_sampling, 'b i j -> b i j ()') == 0)
        att_sum.masked_fill_(att_mask, -torch.finfo(att_sum.dtype).max)                     #以一个极大值进行填充
        att_softmax = att_sum.softmax(dim=-2)                                               #这里是GAT中常用的方法，归一化j边得到归一化权重，随后用这些权重来聚合邻居节点j的value

        att_v = rearrange(l_v, 'b i h d -> b i () h d') * rearrange(r_v, 'b j h d -> b () j h d') * e_v   #这里是将左右两边的value通过广播机制以向量点积的形式进行组合，随后乘以边的value
        att_v = att_softmax.unsqueeze(dim=-1) * att_v                                       #KQ *V ----->[B, N, N, H, D]  这里是将注意力权重应用到value上    
        ...
        coor_out = coor_out * rearrange(complex_graph.flex_coor_mask_after_sampling,    #flex_coor_mask就是用来区分蛋白和protein的mask的
                                        'b i -> b i ()')  # only update ligand coor
#只进行了配体的坐标更新而蛋白质结构坐标保持
#注意力机制的输出处理，生成节点特征，边特征与坐标更新
        if not self.only_coor_out:
            # for node
            node_att = att_softmax    #att_softmax = att_sum.softmax(dim=-2)  att = l_m * r_m * e_m  
            node_att = self.dropout(node_att)
            node_out = torch.einsum('b i j h, b j h d -> b i h d', node_att, torch.cat([l_v, r_v], dim=-1))  #这里就是邻居节点j的value了，此处通过加权求和获得了节点i的消息聚合结果
...
#前向传播用于配体化合物的坐标更新
    def forward(self, complex_graph):                           #shortcut表示原始特征，out_i表示经过门控之后的残差
        # att
